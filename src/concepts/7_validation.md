...
Author-side validation is pretty similar to how data validation works in a traditional client/server app.

### Peer validation

When your node **receives an entry for validation**, the flow is very different. Your Holochain node doesn't just assume that the author has already validated the data; they could easily have hacked their conductor to bypass validation rules. It's your duty and right to treat every piece of data as suspect until you can check it yourself. Fortunately, you have your very own copy of the validation rules.
...
The entry is valid, so they store it in their personal shard of the DHT, along with their **validation signatures** that attest to its validity.
...
The entry is invalid, so they create, sign, and store a **warrant** that contains the evidence (the invalid entry) along with their signatures.

4. ![](https://i.imgur.com/IttfF1u.png)
Norman and Rosie then add Alice to their blacklists and share the warrant with their neighbors, who add her to _their_ blacklists and pass it on.
...
But if your entry holds structured data such as JSON, it's annoying to hand-roll your own parsing code. So the HDK lets you declare a validation function that accepts a [struct](https://doc.rust-lang.org/rust-by-example/custom_types/structs.html) of your own creation. If it sees this sort of function, it'll automatically assume that the entry content is JSON and try to deserialize it into an instance of that struct. If it can't make the conversion, validation fails before your function is even called. This lets you forget about the low-level details and work with your app's native types.
...
* Validation rules are the most important part of a Holochain DNA, as they define the core domain logic that comprise the 'rules of the game'.
...
* An author validates all their entries before committing to their source chain or publishing to the DHT.
