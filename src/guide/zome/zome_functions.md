...
Within the function, `handle_send_message` makes use of a Holochain/HDK function that [sends messages directly node-to-node](https://docs.rs/hdk/latest/hdk/api/fn.send.html).

The available functions, their purpose, and how to use them is fully documented elsewhere, in the [API reference](https://docs.rs/hdk/latest/hdk/api/index.html#functions) and the [List of API Functions](./api_functions.md).
