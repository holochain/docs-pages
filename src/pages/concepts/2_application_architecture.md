...
The origin time defines the earliest possible timestamp that data can have, or in other words the earliest date for which peers will try to create a window, while the quantum time defines the smallest window size. You generally don't need to worry about setting or changing values for the origin and quantum times in a DNA, either when you're creating it or when people are cloning it. But if you find your network isn't synchronizing its data as efficiently as you like, it might be useful to play with these values.
!!!
