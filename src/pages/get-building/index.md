...
At first, each of the UI windows (conductors 0 for Alice and 1 for Bob) are telling us to look at the UI elements that have been generated by the scaffold by looking at the contents in the folder `ui/src/<dna>/<zome>/`, where `<dna>` and `<zome>` are generic placeholders for our DNA (`forum`) and zome (`post`).

Thus far, nine different components have been each stored in a separate `.svelte` file in `ui/src/forum/posts/`. If we go look at that folder we can see the files that have been created.
