...
* a runtime service to manage running applications and provide persistence and networking
...
* Data is held by the users of the app on their own devices. User store their journals, as well as a small slice of others' public data for redundancy.
...
* All public data is visible to anyone with access to the application. However, users can also store some private data in their journals.
...
* Field-based queries are not currently available, though they can potentially be built using existing data primitives.
...
* Individuals take action in the system by running executable code on their own devices, which makes changes to their local state, rather than submitting requests to a global validator network.
...
* A node running multiple application can connect them together through a 'bridging' API.
...
* In addition to being a passive record of events, Holochain also allows interaction between nodes, and between a node and its UI, via 'push' methods such as [RPC and signals](../concepts/8_calls_capabilities/)
...
* Functions are executed only when an individual user says so, and only local state is affected. This state change may then be optionally published to the public graph database so it can be accessed by others.
* Back-end code is typically deployed to interested users as a package of functions, called a [DNA](../concepts/2_application_architecture/). However, a function can call another function in another package if both packages are installed on the same user's device.
...
Holochain is part of an 'agent-centric' category of <abbr title="peer-to-peer">P2P</abbr> <abbr title="distributed web">DWeb</abbr> technologies. It builds an offline-friendly, 'agent-centric', peer-to-peer network on top of cryptographic primitives, similarly to Hypercore, Git, <abbr title="Interplanetary Filesystem">IPFS</abbr>, and Secure Scuttlebutt. 
...
* Holochain is both a protocol and an implementation, and is opinionated about back-end languages and frameworks. Most other stacks have simple protocols and allow developers to choose their own languages and frameworks.
* Holochain, like Secure Scuttlebutt, is optimized for persistent identity, whereas BitTorrent, Hypercore, and IPFS allow for some anonymity.

### What's next?

As our ecosystem matures:

* We'll release new core features and improve our SDK to support developers.
...
* We'll create applications for private data backup, user profile management, and package distribution.
